...
- **Correspondence registration.** Designate a reference mesh (**★** — every error metric is relative to it; there is no absolute ground truth). Promote a pin to a **registration pin** (**⚲**): it auto-seeds one **correspondence marker** per other mesh by closest-point projection of the pin's reference marker, ROI-clamped to the pin. Markers are stored *mesh-local*, so the Before/After toggle moves each marker with its mesh. Refine a marker in the focus panel — click the focused mesh's surface in the 2D view to drop or move its marker (constrained to the pin's region; the click is inverted to a world ray and raycast on the server). With ≥3 markers per moving mesh, **Solve** runs a weighted rigid landmark solve (Umeyama/Arun, server-side) per mesh in parallel and writes each mesh's solved transform. A 3D **constellation** draws each marker, the haloed reference marker, and the lines between them. There is no preview/commit and no undo history — the solve writes the result directly and the Before/After toggle is the comparison.
- **Focus panel (right).** Canvas-only — the main 3D viewport is the only WebGL surface. A **large single** of the focused mesh (pan + zoom, draggable correspondence handle) sits over a **small-multiples** strip (one co-oriented tile per visible mesh), both driven by a **Pano / Top** projection toggle (the displacement channel forces an **Oblique** view). Click a tile to focus that mesh; hold **⇄ ref** to peek the reference in the same frame. The tiles/preview come from a server `mesh-preview` query.
