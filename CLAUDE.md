...
- One shared `Model.Selection` record (`SelectedPin`/`FocusedMesh`/`Hovered`) drives all cross-panel linking. Grammar everywhere: **hover = peek, click = select/promote, drag = edit**. Never add panel-to-panel hover emitters — linked highlighting must stay a consequence of binding to `Selection`.
