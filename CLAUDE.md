...
1. **Effective ghost level (the floor)**: `ghost = GhostOpacity` if `GhostSilhouette` is on (the global **ghost floor**, toggled from the top-bar **👻 Ghost / Hidden** button — also a gear opacity slider), else `0` — with the floor off, ghost fragments are *discarded* (`α < 1e-4` → discard), so "no ghost" means invisible, not translucent. **Solo / peek / isolation all send non-emphasized meshes to whatever the floor is set to**: the fixed peek/isolation dim alphas (0.12 / 0.15) are themselves gated by `GhostSilhouette`, so with the floor off they hide rather than dim (`MeshView` GhostOpacity uniform).
2. **MeshActive** (visibility / isolation): `false` → α = `ghost` for the whole mesh, uniformly.
3. **Pin isolation** (only when pins exist *and* the effective isolation is on): on = the per-mode default `AnchorGhostMode` (set on `SetWorkflowStep` — **on in Correspondence, off in Overview/Inspect**) **OR** the spring-loaded hold modifier `IsolatePeekHeld` (top-bar **◎ Isolate** / hotkey **I**, momentary, mirrors reference-peek). `blobComponent = 1` inside any pin's `InnerRadius`, `0` outside every pin's radius; no pins or off → `1`. During anchor placement `AnchorGhost` is **forced on** regardless and the live hover is appended to `Blobs` as a transient "flashlight" blob (`MeshView.pinBlobUniforms`), so the terrain drops to ghost and only the existing pins + the hover preview read solid.
...
- `DistanceEncoding = 1` — **difference** map on a **soloed moving mesh** (Inspect *Difference* channel): its own `region-distance` signed distance (`Model.FocusDist`), diverging blue↔mid-grey↔red about 0, `DiffRangeScale`-scalable — the same colours as the focus difference tile, now mirrored in the main 3D view when that mesh is soloed.
...
**Clicking a mesh in 3D focuses it** (read/write parity, §B): a plain tap (no placement / no 3D-corr-pick) that lands on a surface runs `View.raycastNearestNamed` — the same parallel cull-and-raycast as `raycastNearest` but keeping the **mesh name** of the nearest hit — and emits `SetFocusedMesh` (in **Inspect** also `ToggleMeshSolo`, §C auto-solo). The focused mesh shows a cyan bbox outline in 3D (`SceneGraph.focusedOutline`), mirroring the rail row + focus tile.
...
**Locate a correspondence** (Correspondence dock manager **⌖**, `FrameCorrespondence`): one atomic action — sets `SelectedPoint`/`FocusedMesh`/`SelectedPin`, **solos** the mesh, **flies the 3D camera tight** to the anchor (`FlyToPoint`), forces `ProjTop`, and **zooms the focus canvas** onto the anchor (`FocusScene.focusOnWorld`, driven from the manager-row handler since `Update` precedes `FocusScene`). It snapshots the prior camera + solo/visibility into `Model.LocateBackup` on the first locate; the focus head's **⤺ back** button (`BackOutLocate`) restores them (and `FocusScene.resetCam`).
...
A **main-3D-view counterpart** does the same pick without the focus panel: each Correspondence-dock row's **⊕ set point** button (`StartCorr3DPick`, `Model.Corr3DPick = Some(pin, mesh)`) **isolates that mesh** in the main viewport (folded into `View.wheelIsolation`, so the rest drop to ghost and the GPU pick lands on the target alone), aims via the same live cyan `CorrPreview` ghost (a **throttled `View.raycastMesh`** single-mesh raycast that falls through ghosts), and commits through the same `PickCorrespondenceAt` (mesh-local, `AnchorPick3D`, exits on commit). Mutually exclusive with everything else: any competing button — place pin, select/delete a pin, focus ⊕, Solve, the row ⟳/⌖, or a workflow-step switch — cancels it (explicit `Corr3DPick = None` in those reducers).
...
3. **Ghost floor** (`GhostSilhouette`/`GhostOpacity`): what non-emphasized geometry renders as — **Ghost** (faint) or **Hidden**. Toggled from the **top-bar 👻 button**; opacity slider in the gear. Solo / peek / isolation all send their others to **this** floor.
...
|---|---|---|---|
| Overview | unified mesh list (hover=peek, click=focus, ★ reference, ● vis, ◐ solo, sensor, ⌖ frame) | WebGL textured tiles | mesh roster |
| Correspondence | pin list + readiness diagnostics | pick surface (Pano/Top) | correspondence manager + Solve |
| Inspect | unified mesh list (click → focus + solo, solve-state flag k/3) + difference sub-mode + intrinsic channels | difference / displacement tiles (channel toggle) | channel toggle + pin distribution + shift readout |

The **rail mesh list is one mode-parameterized component** (`GuiRail.meshRow RowOverview | RowInspect`) — identity / order / selection / hover are identical; only the trailing columns differ (Overview = setup controls incl. ★ reference; Inspect = the solve-state flag ✓ / ready / `k/3` insufficient). **Read/write parity** (§B): clicking a rail row, a focus **tile**, or a **3D mesh** all set `FocusedMesh` (3D via `raycastNearestNamed`, +solo in Inspect); the active mesh shows the matching treatment on the row (`rail-mesh-sel`), the tile (`fm-active`), and in 3D (`focusedOutline` cyan bbox). The rail lists **all** meshes (the control plane includes hidden ones so they can be re-enabled — hidden = dimmed); the tile strip is the **visible** subset (view plane).
...
- `SceneBounds`, `MeshBounds`, `ActivePickingLayer`, `ReferencePeekHeld`, `IsolatePeekHeld` (spring-loaded hold-to-isolate, ◎ / hotkey I)
...
- `WorkflowStep` (`Overview | Correspondence | Inspect`), `InspectChannel` (`ChDifference | ChDisplacement`), `FocusProjection` (`ProjPano | ProjTop`), `FocusPeekReference`, `CorrSetMode` (focus set-correspondence toggle) + `CorrPreview` (live 3D ghost point) + `Corr3DPick` (`(ScanPinId * string) option` — the main-3D-view per-row correspondence pick: isolates that mesh + aims the same ghost), `Toast`
...
GUI placement:
- Top bar (`GuiTopBar`): hamburger, camera reset, **👁 Peek** (hold), **◎ Isolate** (hold), the global **👻 Ghost** floor toggle, the global **Before/After** toggle, gear popover (dataset switch, rendering mode, outline edge threshold + isoline count + difference-heatmap-range sliders, camera speed, ghost silhouette + opacity, isolate-pins, shading strength, slope threshold, quick-pin radius, dataset info, mesh centroids, debug log).
