...
- `HeatmapMode = 1/2/3` — intrinsic incidence / range / triangle-shape false colour, a **per-mesh** setting (`Model.MeshHeatmap : Map<mesh, HeatmapMode>`, set from the **Overview mesh-list** row switches — Tex/Dst/Shp/Inc, `SetMeshHeatmap`). Applied in **every** workflow mode, but **suppressed while that mesh paints an Inspect comparison field** (`DistanceEncoding ≠ 0`) so the 2-mesh / before-after encodings win where they apply (`MeshView` HeatmapMode uniform = per-mesh lookup gated on `distEncoding`; a mesh carrying a heatmap also stays solid in the Inspect no-solo aggregate instead of ghosting). The same three ramps are mirrored in the **2D focus tiles/single** via `FocusShaders.focusColor` (`FocusMode 4/5/6`) over a client-computed per-vertex `FocusScalar` (`FocusScene.focusOverlay` — incidence/range from the mesh-local pano sensor, shape from `MeshView.shapeQuality`), the Inspect difference/displacement channel taking precedence when active. Incidence (angle between the surface normal and the direction to the sensor) and range (distance to the sensor) are both measured from `SensorOrigin` — the **scan sensor** (the mesh's panorama centre, not the interactive camera or the OBJ origin). Shape (per-vertex triangle quality) reads fully green at quality ≥ 0.75 (`v.shq / 0.75f`) so a larger share of a well-formed mesh shows as good.

The variance distances come from `POST /api/query/region-distance` (reference vs each moving mesh) reduced to a per-reference-vertex std by the generation-guarded debounced `Update.ensureVariance` postlude into `Model.SurfaceDistance` (keyed by the reference mesh); the `SurfaceDist` buffer aligns with `loaded.pos`, non-encoded meshes bind a zero buffer of the same length. **Visibility changes invalidate both caches** (`UpdateHelpers.setMeshVisible` clears the variance + bumps the focus-dist generation so newly shown meshes fetch their missing fields); `ensureVariance` is skipped while a mesh is isolated (the aggregate only paints in the no-solo ensemble) and `ensureFocusDist` fetches the shown set (under solo: the isolated mesh, even if its raw toggle is off).
...
- **focus/solo a moving mesh** → central-3D **that mesh's field** (§C): the isolated mesh stays solid and paints its own difference (`DistanceEncoding = 1`, `FocusDist`) or displacement-magnitude (`DistanceEncoding = 3`, client-computed) field in the main 3D view, mirroring its focus tile; **every other mesh — the reference included — drops to the ghost floor** (the co-located reference would occlude the field; its variance encoding is gated to the no-solo ensemble; `MeshView.inspectField` picks the encoding). In Inspect, **focusing a mesh solos it** (reducer policy in `SetFocusedMesh`) and turns pin isolation off; **selecting a pin does the reverse** — exits mesh isolation and turns pin isolation on (`ScanPinMsg SelectPin` policy); a **matrix-cell locate combines both** — solos the mesh *and* turns pin isolation on (`FrameCorrespondence`).
...
**Clicking a mesh in 3D focuses it** (read/write parity, §B): a plain tap (no placement / not armed for correspondence) that lands on a surface runs `View.raycastNearestNamed` — the same parallel cull-and-raycast as `raycastNearest` but keeping the **mesh name** of the nearest hit — and emits `SetFocusedMesh` (the reducer applies the Inspect auto-solo, §C). Raycast candidate sets filter by `MeshVisibility.shown` (toggles + solo overlay), so isolated-away meshes are not click targets. The focused mesh shows a cyan bbox outline in 3D (`SceneGraph.focusedOutline`), mirroring the rail row + focus tile; the **reference** mesh shows a prominent **gold** bbox outline (`SceneGraph.referenceOutline`) + the gold ★ focus tile.
...
**Selection syncs both cameras** (the default, not a toggle): `SetFocusedMesh (Some m)` flies the 3D camera to frame that mesh (the focus single auto-shows it; in **Inspect** the click sites also call `FocusScene.onMeshFocused`, which resets the single to a tight fit of the mesh), `SelectPin (Some id)` flies to the pin centre **and** (from the rail matrix row) zooms the focus single onto the pin at its current mesh (`FocusScene.zoomOnPin`), and a rail matrix cell runs `FrameCorrespondence` + `FocusScene.zoomOnWorldRadius` (2D zoom as tight as the 3D `FlyToPoint`, same metric half-extent).

**Locate a correspondence** (a rail **matrix cell** click, `FrameCorrespondence`): one atomic action — sets `FocusedMesh`/`SelectedPin`, **solos** the mesh (re-enabling its visibility toggle if hidden, so the focus single resolves it), in **Inspect** also turns **pin isolation on** (the located ROI reads solid like a pin click), **flies the 3D camera tight** to the anchor (`FlyToPoint`), forces `ProjTop`, and **zooms the focus canvas** onto the anchor (`FocusScene.zoomOnWorldRadius`, driven from the matrix-cell handler since `Update` precedes `FocusScene`). It snapshots the prior camera + solo/visibility into `Model.LocateBackup` on the first locate; the focus head's **⤺ back** button (`BackOutLocate`) restores them (and `FocusScene.resetCam`).
...
1. **Solo** (`MeshSolo : string option`): a pure **overlay** over the per-mesh visibility toggles — it never mutates `MeshVisible`. `None` → the toggles decide; `Some m` → **only `m`** is shown (in Inspect too — a solid reference would occlude the isolated mesh's field), everything else drops to the **ghost floor**. The one rule is `MeshVisibility.shown` (Model.fs), shared by render `MeshActive`, the raycast candidate sets, and ring/constellation gating. Re-clicking the active ◐ (or switching workflow, or the Inspect pin-focus swap) **exits isolation and resets every visibility toggle to ON** (`UpdateHelpers.exitSolo`); the tile visibility buttons are disabled while any mesh is isolated (reducer-side guard on `SetVisible` too).
...
|---|---|---|---|---|
| Overview | yes | **no** (◐ solos manually) | off | meshes textured, or the per-mesh `MeshHeatmap` intrinsic overlay if set |
| Correspondence | yes | no (locate solos) | **on** | constellation + pin isolation |
| Inspect | yes | **yes** (mesh focus → solo the mesh alone, pin iso off; pin select → solo off, pin iso on; cell locate → solo + pin iso on) | off (+ hold) | no solo → variance aggregate; solo → that mesh's field, everything else at the ghost floor |
...
|---|---|---|---|
| Overview | mesh roster (hover=peek, click name=focus; each row a per-mesh **error-viz switch** Tex/Dst/Shp/Inc = `MeshHeatmap`) | textured tiles — all meshes, each a ★ ref · vis · ◐ isolate strip, hover=peek; no large single | focused-mesh summary card |
| Correspondence | **Solve** (greyed until solvable) + reconstruction-readiness hint, then pin×mesh matrix + place-pin | pick surface (Pano/Top) | pin meta (identity chip · radius · k/n) |
| Inspect | pin×mesh matrix | difference / displacement tiles (channel toggle) | channel toggles (difference sub-mode only; the single-mesh intrinsic switches live in the Overview roster) + distribution + shift readout |
...
GuiTopBar.fs           top bar (isolate/overlays holds, before/after + peek hold, gear popover)
GuiOverlays.fs         toast, scale bar, orientation indicator, wheel label
GuiRail.fs             three-mode left rail (Overview per-mesh error-viz switches; Correspondence Solve + readiness hint)
...
- `MeshHeatmap` (`Map<mesh, HeatmapMode>` — per-mesh intrinsic error viz; absent ⇒ `HeatOff`/textured; set from the Overview roster), `ExtrinsicZDiff` (difference sub-mode M3C2 ↔ Δz), `SurfaceDistance` (`Map<mesh, float32[]>`, the reference variance array), `FocusDist` (`Map<mesh, float32[]>`, per moving mesh signed distance for the focus difference channel)
- `ScanPins` (`ScanPinModel`), `Selection` (`{ SelectedPin; FocusedMesh; Hovered }`)
- `RenderingMode`, `MeshSolo` (`string option` — the isolation overlay, see `MeshVisibility.shown`), `GearPopoverOpen`
...
GUI placement:
- Top bar (`GuiTopBar`): hamburger, **◎ Isolate** (hold), **🎨 Overlays** (hold), the global **Before/After** toggle + spring-loaded **Peek** hold, coordinate readout (friendly mesh name), **⚙ gear** popover (dataset switch, rendering mode, outline edge threshold + isoline count + difference-heatmap-range sliders, camera speed, ghost silhouette + opacity, isolate-pins, shading strength, slope threshold, quick-pin radius, dataset info, mesh centroids, debug log). The **⟲ reset-camera** and **👻 ghost-floor** top-bar buttons were removed (ghost floor lives in the gear; double-tap still recenters).
- Left rail (`GuiRail`): Overview = mesh roster (each row a per-mesh error-viz switch); Correspondence + Inspect = the pin×mesh matrix (Correspondence leads with the **Solve** button — greyed until solvable — plus the reconstruction-readiness hint, then the place-pin button).
- Right focus panel (`GuiFocus` + `FocusScene`): WebGL large-single + per-mesh tiles.
- Bottom dock (`GuiInspector`): Overview = focused-mesh summary; Correspondence = pin meta (identity chip · radius · k/n — Solve moved to the rail); Inspect = channel toggles (difference/displacement + Δ sub-mode; the intrinsic switches moved to the Overview roster) + distribution (all-pin, pin-coloured, brushable) + shift readout.
...
- **OnBoot may run before a *later-sibling* node is mounted.** Don't capture a reference to a following sibling at boot (`el.parentElement.querySelector(...)` stored in a closure) — it can freeze `null` forever and the feature silently no-ops. Look siblings up **lazily** inside the handler that needs them (the brush-chart bridge does `findBridge()` per `emit()`; the focus resize handle resolves `.focus-single` per drag). Boot-time capture is only safe for **ancestors** (`closest(...)`), which are always mounted first.
