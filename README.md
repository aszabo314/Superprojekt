...
- **Survey (Setup tab).** The left navigator opens on the **Setup** view: one row per mesh — colour swatch, number, name. Click a row to fly to that mesh's scan-sensor position; double-click to zoom the 3D camera onto the mesh. Each row carries per-mesh intrinsic heatmap switches (**Tex**tured / **D**i**st**ance range / **Sh**a**p**e quality / **Inc**idence angle). The **★** button designates the mesh as the **reference root** of the registration tree — re-rooting onto a mesh already inside the registered tree *keeps* the registration (path edges reverse), designating an outside mesh clears it.
- **The pair matrix (Pairs tab).** Rows and columns are meshes; the upper-triangular cell (A,B) *is* the pair. Three states: a background hole = the pair doesn't overlap enough to register (checked server-side); an outlined vessel = possible; a filled cell = registered, fill strength = the solve's quality score. The order bar re-sorts rows/columns by sensor order, coverage footprint, or tree distance from the root.
