...
- `MeshVisibility.shown nav name` (Model.fs) is the **single** shown/clickable rule: home = all meshes, cell = the pair only. Every consumer — render `MeshActive`, raycast candidate sets, coverage gating — goes through it; don't special-case visibility anywhere else.
