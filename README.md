...
- **Superserver** — ASP.NET Core + Giraffe. Serves mesh data and runs spatial queries (Embree BVH, closest-point, multi-mesh raycasts, surface patches, sphere contact rings, per-vertex signed distance, weighted correspondence solves, ICP).
...
- **Hover probe.** Ctrl-click any surface for a transient mini-violin chart at the cursor (radius = 5% of the scene bbox diagonal) plus a 3D referent at the hit point — an equator ring sized to the probe radius and a short axis line along the local normal, so a spot-check reads as a region in 3D. The tooltip also lists each mesh's numeric median ± half-IQR. Dismissed by Escape, clicking elsewhere, or after a few seconds.
- **Signed-distance surface map + brushing (3D-linked).** With a pin card open, **⬢ 3D map** colour-maps the soloed (chart-sticky) mesh's per-vertex signed M3C2 distance to the reference straight onto its surface — a diverging blue↔red map centred at 0, with the ±LoD₉₅ band reading neutral, one mesh at a time so overlapping meshes never fight. Hovering the violin draws a **measured ruler** in 3D from the reference surface to the picked distance; *dragging* a y-interval on the violin **brushes** it, keeping that distance band vivid on the surface and washing the rest to context grey (focus+context). The per-vertex distances come from a server query in the served vertex order, so they align exactly with the rendered geometry.
...
- **Ensemble registration (two-stage, preview-first).** Designate a reference mesh (★ in the mesh panel or registration panel — every error metric is relative to it; there is no absolute ground truth). There are two pin kinds: a **scanpin** (measure only) and a **registration pin** (carries one correspondence). **Stage 1 · Correspondence alignment:** turning any pin into a registration pin auto-seeds one **correspondence marker point** per other mesh (closest-point projection of the pin's reference marker) which apply immediately — there is no accept/reject review modal; demoting a registration pin back to a scanpin is the only "reject". Each marker is refinable three ways: co-oriented **patch small-multiples** (orthographic, atlas-textured triangle renderings of every mesh in a shared frame, with a per-cell viridis height ramp, fit-to-footprint zoom + clamped pan; clicking a triangle sets the marker at the exact surface point, and hovering links back into 3D), a **one-shot 3D pick** (target mesh solid, reference at 30 %, everything else ghosted; one depth-gated click, with a guide line along the reference normal showing where the correspondence should land), or **Shift+click on a violin column** (marker at that signed distance along the probe axis). The reference marker itself is editable too. With ≥3 marker pairs per moving mesh, the **Correspondence alignment** action (the green Ready entry in the panel's diagnostics list) runs a weighted rigid solve (Umeyama/Arun, server-side) per mesh in parallel. **Stage 2 · Fine ICP (optional)** only becomes a control once a Stage-1 result is committed: the established ICP (traditional, or region-restricted where pin centres + radii weight the solve), Gauss-Newton point-to-surface with centroid-recentred linearization and trimmed correspondences (3× median gate), starting from the committed transforms. **Both stages land in a pending preview** — meshes render at the previewed pose with their committed pose as a slate ghost, the violin charts split into committed/preview half-violins with a Δ-median arrow, an RMS before → after table with convergence sparklines and collinearity badges sits in the card — until you **Commit** (appends a roll-backable history step) or **Discard**. The newest history step can be rolled back; Reset rolls back everything. Destructive actions (pin placement, retarget, fusion, dataset switch, marker picking) are blocked while a preview is pending.
- **Registration panel.** Top-level `⚲ Registration` opens a one-stop panel over the registration state (it auto-opens the first time you make a registration pin): per-mesh status chips (reference / coarse / fine / skipped) with last-solve RMS and conditioning badges, a per-pin correspondence-marker matrix, a live diagnostics list from a shared readiness engine ("what exactly is missing", each entry with a one-click navigation action, the green Ready entries run the solves), the pending preview banner with commit/discard, and per-mesh error stats with RMS sparklines plus a median-offset-across-pins strip. Rows fly the camera to their mesh/pin (orientation kept). The panel is a pure view — all mutations go through the existing messages and it never issues server queries.
...
- Blob component is 1 inside any pin's `InnerRadius`, 0 outside every pin's range (or 1 if no pins, or if the **Isolate pins** toggle is off).
- α-gated `gl_FragDepth`: in-radius fragments write their natural `gl_FragCoord.z`; ghost / outside fragments write 1.0 (far). Result: translucent ghost geometry doesn't occlude opaque surfaces behind it, but opaque overdraw still works.
- Post-lerp clamp: ghost / outside fragments are pinned below the opaque threshold so only fully-solid surface writes depth.
- 32-slot uniform arrays of lasso half-space planes and pin centres + inner radii `(cx, cy, cz, innerR)`. Pin geometry is stored in metric world-space on the model and converted to render-space (`* datasetScale`) on upload.
