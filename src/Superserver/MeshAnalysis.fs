...
// the mesh surface, traced with the same edge-key marching + linking as the
// isoline above but restricted to BVH candidate triangles. Returns every ring
// (a pin sphere can touch a surface in several places); closed rings repeat
// their first point so rendering has no gap. Points are world-space.
