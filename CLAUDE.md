...
- `NavLevel = NavHome | NavCell of a*b` (`Model.Nav`) + `MatrixHome = HomeOverview | HomePairs` (the Setup|Pairs tabs). `DescendPair` from a possible/registered matrix cell; `NavAscend` is the single backward primitive. There is **no global selection state** — v14 deleted it; never reintroduce one, or panel-to-panel hover/selection emitters.
- `MeshVisibility.shown nav isolate hoverPair name` (Model.fs) is the **single** shown/clickable rule: home = all meshes (the Setup isolate narrows to one, the matrix hover to the hovered pair), cell = the pair only. Every consumer — render `MeshActive`, raycast candidate sets, coverage gating — goes through it; don't special-case visibility anywhere else. The Setup isolate (`SetupIsolate` click-lock + `SetupIsolateHover` button-hover preview; hover wins) and the matrix hover (`MatrixHoverPair` — also drives the shader's per-pixel overlap preview) are tab-scoped transients — the reducer wipes them on tab switch, descend and dataset switch, never a persistent mode.
- ONE `Esc` chain, in the view's key handler: loop modal (cancel) > placement-transaction abort > point-edit disarm > probe disarm > `NavAscend`. New cancellable states slot into this chain, they don't get their own key.
...
- `ErrorRange.ofSamples` (pin samples, always spans 0, hard cap ±0.5 m) is THE shared scale for the 3D map uniforms, the chart x-range and the legend — never per-mesh normalization, no user range sliders.
...
- **V** (visibility: MOV blinks off) and **B** (pose: MOV shows as-loaded) are spring-loaded, cell-scope, refused unless both pair meshes are GPU-resident and no loop modal is open; releases always land; peeks clear on descend/ascend/dataset.
- The peeks are **purely visual**: `MeshView.displayedMeshT`/`displayedWorldAt` are peek-aware (rendering + view-side picks + surface-riding pin markers follow the blink); `ModelTransforms.*` (reducer/query side) is peek-**blind** — no query may read a peeked pose. The vis peek flips `Sg.Active` on all four mesh node families (main surface, outline G-buffer, footprint coverage, suitability) — never the ghost floor, a blink needs a clean swap. Zero refetch: during the pose peek the error map rides MOV's surface with registered-pose values (accepted approximation).
...
One forward pass into the main framebuffer — `passZero`: meshes (custom α + α-gated depth) then pin geometry (`DepthTest.LessOrEqual`, blended); `passOne`: coordinate cross, labels, overlay lines, dark duplex copies (`DepthTest.None`, always on top); `passTwo`: the duplex **white cores**, so they deterministically layer over their dark copies (within-pass order is arbitrary). Offscreen consumers: the image-space outline pass, the footprint coverage MRT and the placement-suitability MRT (active only while the Area tool aims).
...
- The difference map, chart and legend all read `ErrorRange.ofSamples` over the cell's pin samples (see State rules) — signed, spans 0, capped ±0.5 m. Shader uniforms: `DistScale` = hi, `DistLoNeg` = |lo|.
...
- Per-mesh FOOTPRINT contours come from a second, occlusion-free pass: an additive coverage MRT (one channel per mesh, 2×Rgba8, NO depth — cap 8 meshes) + the `OutlineCoverageEdge` composite, which outlines each channel's covered↔uncovered transition in that mesh's palette colour. The depth-tested combined G-buffer can only ever outline the visible **union** (no depth break where one mesh ends over a co-located one; hidden boundaries aren't in the buffer at all) — never remove the coverage pass in favour of it. The MRT renders ONCE per frame (`OutlineView.coverageOffscreen`, shared through SceneGraph) — the footprint composite *and* the forward mesh shader's matrix-hover overlap preview both sample the same textures; a pair mesh beyond the 8-channel cap disables the preview outright (`MeshView.overlapPreviewUniforms`) rather than half-testing.
...
- The placement-suitability MRT reuses the coverage machinery with shape-weighted accumulation; its composite draws before the outline composites so isolines stay readable.
...
- Ghost fragments leave depth at 1.0, so the GPU pixel pick cannot land on them. Anything that needs a 3D point on a possibly-ghosted surface keeps the GPU pick as the fast path and falls back to a server raycast (`View.resolvePick` / `raycastNearest*` / `raycastMesh`) — un-apply the mesh's displayed pose before the query, re-apply it to the hit. Hover-driven raycasts must be throttled (~60–80 ms) + generation-guarded.
...
ScanPinModel.fs / .g.fs ScanPin + placement/edit state
PinGeometry.fs         icosphere, sphere outline
Model.fs / .g.fs       [<ModelType>] Model + Nav + MeshVisibility + ModelTransforms
...
MeshShaders.fs         RenderPass + MeshShader + outline/coverage/suitability shaders
MeshView.fs            LoadedMesh, buildScene, displayed transforms (peek-aware), pin blobs, offscreen nodes
OutlineView.fs         offscreen image-space outline/coverage/suitability passes
ScanPinScene.fs        pin sg nodes: markers, rings, flags, draft, brushed samples
...
GuiRail.fs             left navigator: matrix-home (Setup/Pairs) ⇄ cell workspace (pins, solve, chart)
