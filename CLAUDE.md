...
- Probes are per-pose: `ScanPin.Probe` = the **committed** displayed pose (every consumer — matrix, inspect range, brushing — reads this one); `ScanPin.ProbeOther` = the same probe at the opposite Before/After pose, fetched only once a solve exists, consumed **only** by the violin chart's inactive half. `SetRegView` **swaps** a ready (Probe, ProbeOther) pair in place instead of refetching (and clears `BrushedSamples` — gids index the committed pose's canonical array); everywhere else the two invalidate together (`ScanPinModel.invalidateProbes`).
...
- The dock chart is a **stacked violin** per moving-mesh lane (GuiInspector.fs): histogram bins over the shared x-range, pin layers stacked outward from the lane axis in the canonical pin order (CreatedAt, guid — same as legend + `brushSamples`), smoothed on the *cumulative* outlines so layers never cross. No solve → mirrored halves; solved → **▼ bottom = Before, ▲ top = After**, inactive half muted near-greyscale, Peek flips only the emphasis. One count→height scale across all lanes and both halves.
