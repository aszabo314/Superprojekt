...
type Message =
    ...
    | ToggleReachLog
    // ── Data-state checkpoints (the ⚙ debug menu). The view owns the
    // localStorage IO (save/load/delete/list) and reports back: the stored
    // names after any store operation, and a loaded checkpoint's parsed data.
    ...
    | GraphErrorComputed of gen:int * after:InspectBlock[] * before:InspectBlock[]
    | GraphDistComputed of gen:int * after:(string * float32[])[] * before:(string * float32[])[]
