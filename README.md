...
- **Superserver** — ASP.NET Core + Giraffe. Serves mesh data and runs spatial queries (Embree BVH ray/closest-point, sphere contact rings, surface patches, per-vertex signed distance, co-oriented mesh previews, N-mesh M3C2 probes, and a weighted rigid landmark solve). Also hosts the WASM client.
...
- **Correspondence registration.** Designate a reference mesh (**★** — every error metric is relative to it; there is no absolute ground truth). **Every pin is a registration pin**: on placement it auto-seeds one **correspondence marker** per other mesh by closest-point projection of the pin's reference marker, ROI-clamped to the pin. Markers are stored *mesh-local*, so the Before/After toggle moves each marker with its mesh. Refine a marker in the focus panel — toggle **⊕ set point**, aim by moving the cursor over the focused mesh's 2D view (a live cyan ghost previews the landing point in 3D), and click to place it (constrained to the pin's region; the cursor is inverted to a world ray and raycast on the server). Set mode is off by default so the focus pans freely; placing or toggling off exits it. With ≥3 markers per moving mesh, **Solve** runs a weighted rigid landmark solve (Umeyama/Arun, server-side) per mesh in parallel and writes each mesh's solved transform. A 3D **constellation** draws each marker, the haloed reference marker, and the lines between them. There is no preview/commit and no undo history — the solve writes the result directly and the Before/After toggle is the comparison.
- **Focus panel (right).** Canvas-only — the main 3D viewport is the only WebGL surface. A **large single** of the focused mesh (pan + zoom-to-cursor; **⊕ set point** to aim correspondences) sits over a **small-multiples** strip (one co-oriented tile per visible mesh), both driven by a **Pano / Top** projection toggle (the displacement channel forces an **Oblique** view). Click a tile to focus that mesh; each tile remembers its own pan/zoom, and **⟲ reset** restores the focused tile. Hold **⇄ ref** to peek the reference in the same frame. The tiles/preview come from a server `mesh-preview` query (vertex-cluster decimated so the whole surface shows).
- **Inspect / four error scopes.** Inspect lays out four error scopes at once, all driven by the Before/After toggle. The **central 3D** paints the all-meshes **variance** map (disagreement of every visible moving mesh) on the reference, the moving meshes dropped to faint context. The **focus tiles** show one of two per-mesh channels, toggled in the dock: the **difference** heatmap (signed-distance vs reference — **M3C2** or vertical **Δz** — diverging blue↔red about 0 with a ±LoD₉₅ neutral band on a shared colour scale), or the **displacement** glyph field (white surface + load→solved arrows coloured *and* lengthed by magnitude, oblique projection, for solved meshes only). The bottom **dock** holds the selected pin's **distribution** (jittered raw probe samples + median/IQR box on a shared signed-distance axis with the ±LoD₉₅ band) and, under the displacement channel, a **shift readout** (the focused mesh's centroid displacement split into vertical-datum / horizontal + rotation angle, derived from its solved transform). The pre-existing intrinsic **incidence / range / triangle-shape** acquisition heatmaps stay available in the rail.
...
MeshPreview.fs   co-oriented mesh preview for the focus panel
