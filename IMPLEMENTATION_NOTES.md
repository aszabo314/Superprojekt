...
| A4 live landing marker / ridge emphasis | deferred (optional stretch) | Spec marks both as stretch ("emphasis alone satisfies the request", behind a flag). The **predicted correspondence** is already shown: the auto-seeded (closest-point) anchor for the mesh being picked renders as a tetra glyph + a line to the reference marker, and the A4 guide line shows the reference normal — so the landing point is visible without a dedicated per-cursor raycast. **Ridge/curvature emphasis** (highlight high-curvature vertices to aid placement) is the one genuinely-unbuilt piece; low marginal value, available on request. |
