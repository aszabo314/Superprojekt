...
- 3D marks are **duplex**: a white core with a near-black ink outline (`LineGlyphs.duplex`), readable on any terrain. All-white (no ink) is the *uncommitted* layer — pin drafts and aim previews — plus ONE committed exception: the pin's sphere∩surface contact rings render single-stroke white (a deliberate user choice). Committed pin point markers = mesh-colour fill + white outline.
